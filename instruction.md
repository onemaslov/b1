...
## Основная функциональность:
    ...
    ✅ Автоматическое определение геолокации пользователя при загрузке
    ✅ Кнопка для быстрого перехода к текущему местоположению
    ✅ Визуальное отображение местоположения пользователя на карте
    ✅ Умный поиск на карте:
       - Поиск по адресу с автоподсказками
       - Поиск по координатам (широта, долгота)
       - Автоматическая установка метки при выборе результата
...
│   │   │   └── geocode/
│   │   │       └── route.ts   # GET (поиск адресов через Nominatim)
│   │   │
│   │   ├── globals.css        # Глобальные стили (Tailwind + Leaflet + анимации)
│   │   ├── layout.tsx         # Корневой layout с метаданными
│   │   └── page.tsx           # Главная страница приложения
│   │                          # Содержит логику управления состоянием,
│   │                          # загрузку меток, геолокацию, поиск, обработчики событий
│   │
...
│   │   │                      # - Программное центрирование (для поиска)
│   │   │
...
│   │   │
│   │   ├── LocationButton.tsx # Кнопка геолокации (справа внизу)
...
│   │
│   ├── hooks/
│   │   └── useGeolocation.ts  # React хук для работы с геолокацией
...
│   │   └── geocoding.ts       # Функции для работы с геокодированием
...
- Использует Geolocation API браузера
- Автоматически запрашивает разрешение при загрузке (опционально)
- Возвращает координаты, состояние загрузки и ошибки
- Обрабатывает все типы ошибок (отказ, недоступность, таймаут)
- Настраиваемая точность и таймауты

**`LocationButton.tsx`**
- Плавающая кнопка в правом нижнем углу
- Меняет цвет в зависимости от статуса разрешения
- Показывает индикатор загрузки при определении местоположения
- Плавные анимации при наведении

**Работа геолокации:**
1. При первой загрузке страницы запрашивается разрешение
2. Если разрешено - карта центрируется на местоположении пользователя
3. На карте появляется синий маркер с текущим местоположением
4. Кнопка позволяет повторно центрировать карту
5. При ошибках показывается Toast уведомление
...
- `GET` - поиск адресов через Nominatim API (OpenStreetMap)
- Параметры: `?q=<поисковый запрос>`
- Возвращает до 10 результатов с координатами и адресами
- Поддержка русского языка

### Frontend (Components)

**`SearchBar.tsx`**
- Компонент поиска на карте в верхней части экрана
- Поддерживает поиск по адресу и координатам
- Автоподсказки при вводе адреса (debounce 500ms)
...
6. **Поиск на карте**
   - Ввод адреса в поле поиска вверху страницы
   - Появление автоподсказок с адресами
   - Клик на результат → центрирование + временный маркер
   - Ввод координат (широта, долгота)
   - Enter → центрирование + открытие модального окна метки
